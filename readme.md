...
```sh
$ git rebase <rebase先のブランチ名>
```

## コミットのやり直し
実際にgitを使っていると、もちろん間違えて`commit`することもある。あるいは、過去に行った`commit`が後々になって不要になるということもある。そのような場合に役に立つ操作もある。

### reset
現在のブランチのリビジョンを、特定のリビジョンに戻す。
例えば、mainブランチをrev.main2に`reset`すると、
`reset`前
```
[rev.main1] --> [rev.main2] --> [rev.main3] --> [rev.main4] # main
```
...
となる。\
このとき、rev.main3とrev.main4の更新内容がどうなるかは`reset`時のオプションによって異なる。
* soft
* mixed
* hard

`soft`オプションはリビジョン自体はなくなるが、ファイルの更新は残る（ファイル実体はrev.main4のまま）し、[ステージング](#add)の状態も維持される。\
`mixed`オプションはリビジョン自体はなくなり、ファイルの更新は残る。そしてステージングの状態はなくなる。\
`hard`オプションはリビジョンも更新もステージングの状態も全てなくなる。\
困ったときには、取り敢えず`soft`オプションを使うと良い。

```sh
$ git reset [--soft|--hard|--mixed] <戻すリビジョンのハッシュ値>
```
